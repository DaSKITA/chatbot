...
class ActionSetSlotValueRequestService(Action):
    ...
    def run(self,
        ...
        if slot_value_service_company and slot_value_service_company[0] in service_filter.service_list:
            ...
            if channel =="telegram": #for telegram format differently
                ...
                if datatype=="countries" and len(service_list)>1: #if more than one service given
                    ...
                    if not categories:
                        ...
                        categories_string = '<br> '.join([str(elem) for elem in categories if elem != ""])
                        sub_categories = element.get("subcategories", None)
                        if sub_categories:
                            sub_categories_string = ', '.join([str(elem) for elem in sub_categories if elem != ""])
                            text_str = "Der Dienst {} speichert folgende Daten: {}.\nDiese Daten schließen mit ein: {}".format(service_upper, categories_string, sub_categories_string)
                            dispatcher.utter_message(text=text_str)
                        else:
